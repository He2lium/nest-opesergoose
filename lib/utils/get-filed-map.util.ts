...
export function GetFiledMapUtil(options: MappingProperty) {
  if (
    (options as MappingPropertyBaseWithPropertiesFromClass).propertiesFromClass
  ) {
    const nestedSchema: schema = OpeserMappingStorage.getSchemaByClass(
      (options as MappingPropertyBaseWithPropertiesFromClass)
        .propertiesFromClass.name
    );
    const map = {
      ...options,
